package banking.account.constant;

public class ConstantMessage {
    public static final String CHOICE = "아래 기능 중 하나를 선택하세요.";
    public static final String MENU= "(1) : 계좌 생성  (2) : 입금 (3) : 출금 (4) : 송금 (5) 계좌조회";
    public static final String ACCOUNT_TYPE= "(1) : 예금 계좌 , (2) : 적금 계좌";
    public static final String ACCOUNT_NUMBER = "계좌번호를 입력하세요. (8자리 입력)";
    public static final String ACCOUNT_OWNER = "이름을 입력하세요.";
    public static final String BALANCE = "금액을 입력하세요.";

    public static final String TARGET = "목표 ";
    public static final String DEPOSIT = "입금 ";
    public static final String WITHDRAW = "출금 ";
    public static final String TRANSFER = "송금 ";

    public static final String ACCOUNT_INFO = "계좌의 정보는 다음과 같습니다.";

    public static final String ERROR_INPUT_TYPE = "입력 형식이 올바르지 않습니다. 숫자만 입력해주세요.";


}
